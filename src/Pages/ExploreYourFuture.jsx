...
const stats = (() => {
  ...
  const highPayingJobs = mockJobs.filter((job) => {
    ...
    return firstAmount && Number.parseInt(firstAmount) >= 100000
  })
...
const jobCategories = (() => {
  ...
  mockJobs.forEach((job) => {
    ...
    if (salaryMatch?.length >= 2) {
      const min = Number.parseInt(salaryMatch[0].replace(/[$,]/g, ""))
      const max = Number.parseInt(salaryMatch[1].replace(/[$,]/g, ""))
...
const JobCategoryCard = ({ category, onViewJobs }) => (
...
const ApplySection = ({ section, onViewJobs, isReversed = false }) => {
  const visualTitles = { Entry: "Start Your Journey", High: "Premium Positions", Remote: "Work Remotely" }
  const visualDescriptions = {
    Entry: "Build your career with guidance",
    High: "Top-tier compensation",
    Remote: "Location independence",
  }

  const getVisualTitle = () =>
    Object.entries(visualTitles).find(([key]) => section.title.includes(key))?.[1] || "Flexible Options"
  const getVisualDescription = () =>
    Object.entries(visualDescriptions).find(([key]) => section.title.includes(key))?.[1] || "Work your way"
  ...
  const content = (
    <div className="text-white">
      ...
      <button
        ...
        aria-label={`${section.features ? "View Available Jobs" : "View Available Jobs"} in section`}
      >
        {section.features ? "View Available Jobs" : "View Available Jobs"}
      </button>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-8 text-center">
          ...
          <h2 className="text-xl md:text-2xl font-semibold text-gray-700 mb-6">
            Your Gateway to the Perfect Career Opportunity
          </h2>
          <p className="text-lg text-gray-600 max-w-4xl mx-auto leading-relaxed">
            We are a premier career portal dedicated to connecting professionals with the best job opportunities.
            Whether you are a fresh graduate, an experienced professional, or someone looking for a career change, we
            help you find the perfect position that matches your skills and ambitions. Currently featuring{" "}
            {stats.totalJobs} active opportunities across {stats.companies} leading companies.
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="text-white">
              ...
              <p className="text-lg mb-8 text-blue-100">
                ...
                executive roles, find your perfect match among our {stats.totalJobs} current openings at{" "}
                {stats.companies} top companies.
              </p>
              <button
      ...
      {EXPLORE_DATA.sections.map((section, index) => (
        <ApplySection key={index} section={section} onViewJobs={handleViewJobs} isReversed={index % 2 === 1} />
      ))}
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {jobCategories.map((category) => (
              <JobCategoryCard key={category.id} category={category} onViewJobs={handleViewJobs} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="py-16 bg-blue-600">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-xl text-blue-100 mb-8">
            Join thousands of professionals who have found their dream jobs through our platform. With {stats.totalJobs}{" "}
            active positions and {stats.companies} top companies, your next opportunity is waiting.
          </p>
